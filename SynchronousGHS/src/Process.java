...
public class Process implements Runnable {
	...
	int[][] arrCoveredID;
	int[][] arrNotCoveredID;
	int[] arrToSendCovered, arrToSendNotCovered, arrChildrenID;
...
//		System.out.println("Number of neighbors for Process id-"+iMyPID+" is: "+iNumberOfNbrs);
		...
		//Initialize array to store the id's of the children
		arrChildrenID=new int[iNumberOfNbrs];
...
//			System.out.println("I have no parent. I'm a potential leader-"+iMaxPID);
	...
	void reset(String sColumnToReset){
		...
		case "parent":
			...
			bShouldWeLoop=true;
			break;
			//		Reset the array of children id to 0
		case "arrChildrenID":
			for(iCount=1;iCount<=iNumberOfNbrs;iCount++){
				arrChildrenID[iCount-1]=0;
			}
			bShouldWeLoop=false;
			break;
		case "receivedMaxID":
...
//		System.out.println("Start of updateChildrenList-"+tProcess.getName());
		int iCount, iIndex=0;
		reset("arrChildrenID");
		for(iCount=1;iCount<=iNumberOfNbrs;iCount++){
			if(mCon[iCount-1][2]==2){
				iIndex++;
				arrChildrenID[iIndex-1]=mCon[iCount-1][1];
			}
		}
//		System.out.println("End of updateChildrenList-"+tProcess.getName());
	}

	public void printChildren(){
		//		System.out.println("Start of printChildren()"+tProcess.getName());
		int iCount;
		for(iCount=1;iCount<=iNumberOfNbrs;iCount++){
			if(arrChildrenID[iCount-1] != 0){
				if(iCount-1 > 0){
					System.out.print(",");
				}
				System.out.print(arrChildrenID[iCount-1]);
			}
			else{
				break;
			}
		}
		//		System.out.println("End of printChildren()"+tProcess.getName());
	}
	...
	public void printParent(){
		if(iParentIndex != -1){
			System.out.print(mCon[iParentIndex-1][1]);
		}
		else{
