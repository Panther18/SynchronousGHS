...
public class Master implements Runnable{
	...
	public void getInput()
	{
		...
		for(iRow=1;iRow<=iNumberOfProcesses;iRow++){
			...
			for(iCol=iRow+1;iCol<=iNumberOfProcesses;iCol++)
			{
				//if(iRow!= iCol)
					{
			...
			System.out.println("");
			for(iCol=1;iCol<=iNumberOfProcesses;iCol++)
			{
				if(iGraph[iCol-1][iRow-1]==0)
				{
					checkisolatednode++;
				}
			}
			if(checkisolatednode==iNumberOfProcesses)
			{
			checkisolatedtree++;
			//System.out.println(checkisolatedtree);
				System.out.println("There are no neighbour process node connected for process " + iPID[iRow-1]);
